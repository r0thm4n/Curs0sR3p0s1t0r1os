...
# 14. Operador de propagación

El operador de propagación (...) en JavaScript permite "expandir" elementos de un iterable (como un array, un objeto o una cadena) en lugares donde se esperan múltiples elementos o valores. Es especialmente útil para combinar, clonar y manipular arrays y objetos de manera más legible y eficiente.

###### Ejemplos de cómo se usa el operador de propagación:
1. En Arrays:
...
const arr1 = [1, 2, 3];
const arr2 = [4, 5, 6];
const arr3 = [...arr1, ...arr2]; // Concatenamos ambos arrays
...
const copia = [...original]; // Clonamos el array

copia.forEach(element => console.log(element));
// Salida en consola:
// 1
// 2
// 3
...
- **Ejemplo 3: Combinar Objetos y recorrer las propiedades**

Aunque el operador de propagación `(...)` se usa en arrays y objetos, el método `forEach` no se aplica directamente a objetos. Para recorrer un objeto, puedes usar `Object.entries`, `Object.keys` o `Object.values`.

```javascript
const obj1 = { nombre: "Goku" };
const obj2 = { poder: "Kamehameha" };
const combinado = { ...obj1, ...obj2 }; // Combinamos ambos objetos
...
- **Ejemplo 4: Pasar elementos de un array como argumentos y ver el resultado en la consola**
...
console.log(sumar(...numeros)); // Salida en consola: 6
```

Aquí no estamos usando `forEach` directamente, pero puedes ver cómo el operador de propagación permite pasar los valores del array `numeros` como argumentos individuales para la función `sumar`.
